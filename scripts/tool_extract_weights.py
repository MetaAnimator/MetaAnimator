...
def main():
    ...
    if args.type == 'lora':
        if not args.from_base:
            ...
            print("tasks list:", model.control_model.tasks)
            for task in model.control_model.tasks:
                ...
                os.makedirs(args.save_path.replace(".ckpt", ""), exist_ok=True)
                save_path = os.path.join(args.save_path.replace(".ckpt", ""), f'{task}.ckpt')
